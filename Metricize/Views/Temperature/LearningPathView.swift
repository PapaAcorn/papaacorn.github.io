...
struct LearningPathNode: Identifiable {
    ...
    let isFinalExam: Bool
    let isRoundStart: Bool
    let state: LearningPathNodeState

    var id: String {
        isFinalExam ? "exam" : "\(roundIndex)-\(subRoundIndex)"
    }

    var label: String {
        TemperatureCurriculum.pathLabel(roundIndex: roundIndex, subRoundIndex: subRoundIndex)
    }
}
...
struct LearningPathView: View {
    ...
    private var nodes: [LearningPathNode] {
        TemperatureCurriculum.rounds.flatMap { round -> [LearningPathNode] in
    ...
    private func accessibilityLabel(for node: LearningPathNode) -> String {
        ...
        let name = node.isFinalExam ? "Final exam" : "Round \(node.label)"
        return "\(name), \(status)"
    }
}
