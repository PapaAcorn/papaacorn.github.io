//
//  KitchenLearningPathView.swift
//  Metricize
//

import SwiftUI

struct KitchenLearningPathView: View {
    ...
    private var nodes: [LearningPathNode] {
        KitchenCurriculum.rounds.flatMap { round -> [LearningPathNode] in
    ...
    private func pathNode(_ node: LearningPathNode) -> some View {
        ...
        let nodeBody = VStack(spacing: metrics.labelSpacing) {
            ...
            if style == .standard || node.isRoundStart || node.isFinalExam {
                Text(KitchenCurriculum.pathLabel(roundIndex: node.roundIndex, subRoundIndex: node.subRoundIndex))
    ...
    private func accessibilityLabel(for node: LearningPathNode) -> String {
        ...
        let name = node.isFinalExam ? "Final exam" : "Round \(KitchenCurriculum.pathLabel(roundIndex: node.roundIndex, subRoundIndex: node.subRoundIndex))"
        return "\(name), \(status)"
    }
    ...
    private func pathConnector(from previous: LearningPathNode, to next: LearningPathNode) -> some View {
        ...
        return Capsule()
            .fill(isActive ? MetricTheme.warmEmber.opacity(0.55) : palette.progressTrack)
