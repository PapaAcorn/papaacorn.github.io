//
//  HereToThereMultipleChoiceChallengeView.swift
//  Metricize
//

import SwiftUI

struct HereToThereMultipleChoiceChallengeView: View {
        ...
        Group {
            ...
            case .stacked:
                VStack(spacing: compact ? 20 : 32) {
                    HereToTherePromptView(card: card, compact: compact)
                    choiceList
                }
            case .sideBySide:
...
private struct RoadChoiceButton: View {
    let label: String
    let isEnabled: Bool
    let delay: Double
    let appeared: Bool
