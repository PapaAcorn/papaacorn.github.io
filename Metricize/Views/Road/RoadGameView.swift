...
struct RoadGameView: View {
    @State private var viewModel: RoadGameViewModel
    ...
    init(
        progressStore: RoadProgressStore,
        settings: AppSettingsStore,
        onResetModule: (() -> Void)? = nil,
        onReviewBasics: (() -> Void)? = nil
    ) {
        _viewModel = State(
            initialValue: RoadGameViewModel(progressStore: progressStore, settings: settings)
        )
    ...
    private var roundLabel: String {
        ...
        return "Round \(RoadCurriculum.subRoundLabel(majorRoundIndex: major, subRoundIndex: minor))"
    }
    ...
    var body: some View {
        ...
        .confirmationDialog(
            "Reset all progress for On the Road?",
    ...
    private func tipTitle(for roundIndex: Int) -> String {
        ...
        return "Round \(roundIndex + 1): \(RoadCurriculum.roundTitle(for: roundIndex))"
    }

    private var progressStore: RoadProgressStore {
        viewModel.progressStore
    }

    @ViewBuilder
    private func challengeView(for card: RoadCard, isLandscape: Bool) -> some View {
        GeometryReader { geometry in
            ...
            ScrollView(.vertical, showsIndicators: false) {
                VStack(spacing: 0) {
                    ...
                    RoadMultipleChoiceChallengeView(
                        card: card,
    ...
    private func prepareChallenge(for card: RoadCard) {
    ...
    private var moduleCompleteView: some View {
        VStack(spacing: 0) {
            ...
            Text("You've passed the final exam and can quickly recognize the road speeds, distances, and travel-time anchors most likely to help while traveling in metric countries.")
                .font(.body)
                .foregroundStyle(palette.textSecondary)
