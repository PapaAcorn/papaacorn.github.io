//
//  RoadLearningPathView.swift
//  Metricize
//

import SwiftUI

struct RoadLearningPathView: View {
    ...
    private var nodes: [LearningPathNode] {
        RoadCurriculum.rounds.flatMap { round -> [LearningPathNode] in
    ...
    private func pathNode(_ node: LearningPathNode) -> some View {
        ...
        let nodeBody = VStack(spacing: metrics.labelSpacing) {
            ...
            if style == .standard || node.isRoundStart || node.isFinalExam {
                Text(RoadCurriculum.pathLabel(roundIndex: node.roundIndex, subRoundIndex: node.subRoundIndex))
    ...
    private func accessibilityLabel(for node: LearningPathNode) -> String {
        ...
        let name = node.isFinalExam ? "Final exam" : "Round \(RoadCurriculum.pathLabel(roundIndex: node.roundIndex, subRoundIndex: node.subRoundIndex))"
        return "\(name), \(status)"
    }
