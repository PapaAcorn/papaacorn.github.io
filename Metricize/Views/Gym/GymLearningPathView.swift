//
//  GymLearningPathView.swift
//  Metricize
//

import SwiftUI

struct GymLearningPathView: View {
    ...
    private var nodes: [LearningPathNode] {
        GymCurriculum.rounds.flatMap { round -> [LearningPathNode] in
    ...
    private func pathNode(_ node: LearningPathNode) -> some View {
        ...
        let nodeBody = VStack(spacing: metrics.labelSpacing) {
            ...
            if style == .standard || node.isRoundStart || node.isFinalExam {
                Text(GymCurriculum.pathLabel(roundIndex: node.roundIndex, subRoundIndex: node.subRoundIndex))
    ...
    private func accessibilityLabel(for node: LearningPathNode) -> String {
        ...
        let name = node.isFinalExam ? "Final exam" : "Round \(GymCurriculum.pathLabel(roundIndex: node.roundIndex, subRoundIndex: node.subRoundIndex))"
        return "\(name), \(status)"
    }
