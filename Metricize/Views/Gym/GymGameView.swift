...
struct GymGameView: View {
    @State private var viewModel: GymGameViewModel
    ...
    init(
        progressStore: GymProgressStore,
        settings: AppSettingsStore,
        onResetModule: (() -> Void)? = nil,
        onReviewBasics: (() -> Void)? = nil
    ) {
        _viewModel = State(
            initialValue: GymGameViewModel(progressStore: progressStore, settings: settings)
        )
    ...
    private var roundLabel: String {
        ...
        return "Round \(GymCurriculum.subRoundLabel(majorRoundIndex: major, subRoundIndex: minor))"
    }
    ...
    var body: some View {
        ...
        .confirmationDialog(
            "Reset all progress for At the Gym?",
    ...
    private func tipTitle(for roundIndex: Int) -> String {
        ...
        return "Round \(roundIndex + 1): \(GymCurriculum.roundTitle(for: roundIndex))"
    }

    private var progressStore: GymProgressStore {
        viewModel.progressStore
    }

    @ViewBuilder
    private func challengeView(for card: GymCard, isLandscape: Bool) -> some View {
        GeometryReader { geometry in
            ...
            ScrollView(.vertical, showsIndicators: false) {
                VStack(spacing: 0) {
                    ...
                    GymMultipleChoiceChallengeView(
                        card: card,
    ...
    private func prepareChallenge(for card: GymCard) {
    ...
    private var moduleCompleteView: some View {
        VStack(spacing: 0) {
            ...
            Text("You've passed the final exam and can quickly recognize body weights, dumbbells, barbells, machine stacks, plates, and cardio speeds you're likely to see in metric gyms.")
                .font(.body)
                .foregroundStyle(palette.textSecondary)
