...
struct ShopGameView: View {
    @State private var viewModel: ShopGameViewModel
    ...
    init(
        progressStore: ShopProgressStore,
        settings: AppSettingsStore,
        onResetModule: (() -> Void)? = nil,
        onReviewBasics: (() -> Void)? = nil
    ) {
        _viewModel = State(
            initialValue: ShopGameViewModel(progressStore: progressStore, settings: settings)
        )
    ...
    private var roundLabel: String {
        ...
        return "Round \(ShopCurriculum.subRoundLabel(majorRoundIndex: major, subRoundIndex: minor))"
    }
    ...
    var body: some View {
        ...
        .confirmationDialog(
            "Reset all progress for In the Shop?",
    ...
    private func tipTitle(for roundIndex: Int) -> String {
        ...
        return "Round \(roundIndex + 1): \(ShopCurriculum.roundTitle(for: roundIndex))"
    }

    private var progressStore: ShopProgressStore {
        viewModel.progressStore
    }

    @ViewBuilder
    private func challengeView(for card: ShopCard, isLandscape: Bool) -> some View {
        GeometryReader { geometry in
            ...
            ScrollView(.vertical, showsIndicators: false) {
                VStack(spacing: 0) {
                    ...
                    ShopMultipleChoiceChallengeView(
                        card: card,
    ...
    private func prepareChallenge(for card: ShopCard) {
    ...
    private var moduleCompleteView: some View {
        VStack(spacing: 0) {
            ...
            Text("You've passed the final exam and can quickly recognize store weights, grocery quantities, bottle sizes, clothing measurements, and product dimensions while shopping in metric countries.")
                .font(.body)
                .foregroundStyle(palette.textSecondary)
