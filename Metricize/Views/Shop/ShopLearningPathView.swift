//
//  ShopLearningPathView.swift
//  Metricize
//

import SwiftUI

struct ShopLearningPathView: View {
    ...
    private var nodes: [LearningPathNode] {
        ShopCurriculum.rounds.flatMap { round -> [LearningPathNode] in
    ...
    private func pathNode(_ node: LearningPathNode) -> some View {
        ...
        let nodeBody = VStack(spacing: metrics.labelSpacing) {
            ...
            if style == .standard || node.isRoundStart || node.isFinalExam {
                Text(ShopCurriculum.pathLabel(roundIndex: node.roundIndex, subRoundIndex: node.subRoundIndex))
    ...
    private func accessibilityLabel(for node: LearningPathNode) -> String {
        ...
        let name = node.isFinalExam ? "Final exam" : "Round \(ShopCurriculum.pathLabel(roundIndex: node.roundIndex, subRoundIndex: node.subRoundIndex))"
        return "\(name), \(status)"
    }
