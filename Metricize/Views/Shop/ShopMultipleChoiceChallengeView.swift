//
//  ShopMultipleChoiceChallengeView.swift
//  Metricize
//

import SwiftUI

struct ShopMultipleChoiceChallengeView: View {
    let card: ShopCard
    let choices: [Int]
        ...
        Group {
            ...
            case .stacked:
                VStack(spacing: compact ? 20 : 32) {
                    ShopPromptView(card: card, compact: compact)
                    choiceList
                }
            case .sideBySide:
...
private struct ShopChoiceButton: View {
    let label: String
    let isEnabled: Bool
    let delay: Double
    let appeared: Bool
