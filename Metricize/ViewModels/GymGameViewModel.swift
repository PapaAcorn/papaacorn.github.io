...
enum GymGamePhase: Equatable {
    case playing(GymCard)
...
enum GymAnswerFeedback: Equatable {
    case none
    case exact(answerLabel: String, card: GymCard)
    case incorrect(correctLabel: String, card: GymCard)
}

@Observable
@MainActor
final class GymGameViewModel {
    let progressStore: GymProgressStore
    let settings: AppSettingsStore

    private(set) var phase: GymGamePhase = .playing(GymCurriculum.rounds[0].cards[0])
    private(set) var feedback: GymAnswerFeedback = .none
    private(set) var isSubmitting = false
    private(set) var activeChoiceLabels: [String]?
    private var activeCorrectChoiceIndex = 0
    private var lastCardID: String?

    init(progressStore: GymProgressStore, settings: AppSettingsStore) {
    ...
    var currentRound: GymRound {
        GymCurriculum.rounds[progressStore.currentRoundIndex]
    }
    ...
    func submitMultipleChoice(guess: Int, for card: GymCard) {
    ...
    private func resumeFinalExam() {
        if progressStore.shouldShowTipBeforeRound(GymGameConstants.finalExamRoundIndex) {
            showTip(for: GymGameConstants.finalExamRoundIndex)
            return
        }
    ...
    private func submitAnswer(guess: Int, for card: GymCard) {
        ...
        let result = GymConversion.evaluate(guess: guess, correctIndex: activeCorrectChoiceIndex)
    ...
    private func submitExamAnswer(guess: Int, for card: GymCard) {
    ...
    private func presentFeedback(
        result: GymConversion.AnswerResult,
        card: GymCard,
    ...
    private func continueAfterLearningAnswer(for card: GymCard) {
    ...
    private func presentFinalExamResult(from session: GymFinalExamSession) {
        let passed = session.correctCount >= GymGameConstants.examPassCorrectCount
        if passed {
    ...
    private func presentBatchReviewIfNeeded() -> Bool {
        ...
        phase = .showingBatchReview(
            roundIndex: roundIndex,
            subRoundIndex: subRoundIndex,
            items: GymCurriculum.batchReviewItems(for: cards)
        )
        return true
    }

    private func transitionToCard(_ card: GymCard) {
        if isTakingFinalExam {
            showCurrentExamCard()
            return
        }
    ...
    private func showTip(for roundIndex: Int) {
        phase = .showingTip(
            roundIndex: roundIndex,
            tips: GymCurriculum.tips(forRound: roundIndex)
        )
    }

    private func selectNextCard(excluding excludedID: String? = nil) -> GymCard? {
        ...
        if progressStore.currentSubRoundIndex == GymGameConstants.mixedSubRoundIndex {
        ...
        var pool: [(card: GymCard, weight: Int)] = []
        ...
        for card in unlearnedCurrent where card.id != avoidID {
            ...
            if progress.totalIncorrect > 0 && progress.consecutiveCorrect == 0 {
                weight = GymGameConstants.strugglingCardWeight
            } else if progress.consecutiveCorrect > 0 {
                weight = GymGameConstants.partialProgressWeight
            } else {
                weight = GymGameConstants.currentRoundCardWeight
            }
            pool.append((card, weight))
        }

        if pool.isEmpty {
            pool = unlearnedCurrent.map { ($0, GymGameConstants.currentRoundCardWeight) }
        }

        if progressStore.currentSubRoundIndex == GymGameConstants.mixedSubRoundIndex {
            let reviewCards = GymCurriculum.allCards.filter { reviewCard in
            ...
            for card in reviewCards {
                pool.append((card, GymGameConstants.reviewCardWeight))
            }
        }
    ...
    private func isEarlierSubRound(_ card: GymCard) -> Bool {
    ...
    private func weightedRandom(from pool: [(card: GymCard, weight: Int)]) -> GymCard? {
...
extension GymGameViewModel {
    func multipleChoiceOptions(for card: GymCard) -> [Int] {
        let answerLabel = card.answerLabel
        let pool = GymConversion.distractorLabels(for: card).filter { $0 != answerLabel }
