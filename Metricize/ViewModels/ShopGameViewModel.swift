...
enum ShopGamePhase: Equatable {
    case playing(ShopCard)
...
enum ShopAnswerFeedback: Equatable {
    case none
    case exact(answerLabel: String, card: ShopCard)
    case incorrect(correctLabel: String, card: ShopCard)
}

@Observable
@MainActor
final class ShopGameViewModel {
    let progressStore: ShopProgressStore
    let settings: AppSettingsStore

    private(set) var phase: ShopGamePhase = .playing(ShopCurriculum.rounds[0].cards[0])
    private(set) var feedback: ShopAnswerFeedback = .none
    private(set) var isSubmitting = false
    private(set) var activeChoiceLabels: [String]?
    private var activeCorrectChoiceIndex = 0
    private var lastCardID: String?

    init(progressStore: ShopProgressStore, settings: AppSettingsStore) {
    ...
    var currentRound: ShopRound {
        ShopCurriculum.rounds[progressStore.currentRoundIndex]
    }
    ...
    func submitMultipleChoice(guess: Int, for card: ShopCard) {
    ...
    private func resumeFinalExam() {
        if progressStore.shouldShowTipBeforeRound(ShopGameConstants.finalExamRoundIndex) {
            showTip(for: ShopGameConstants.finalExamRoundIndex)
            return
        }
    ...
    private func submitAnswer(guess: Int, for card: ShopCard) {
        ...
        let result = ShopConversion.evaluate(guess: guess, correctIndex: activeCorrectChoiceIndex)
    ...
    private func submitExamAnswer(guess: Int, for card: ShopCard) {
    ...
    private func presentFeedback(
        result: ShopConversion.AnswerResult,
        card: ShopCard,
    ...
    private func continueAfterLearningAnswer(for card: ShopCard) {
    ...
    private func presentFinalExamResult(from session: ShopFinalExamSession) {
        let passed = session.correctCount >= ShopGameConstants.examPassCorrectCount
        if passed {
    ...
    private func presentBatchReviewIfNeeded() -> Bool {
        ...
        phase = .showingBatchReview(
            roundIndex: roundIndex,
            subRoundIndex: subRoundIndex,
            items: ShopCurriculum.batchReviewItems(for: cards)
        )
        return true
    }

    private func transitionToCard(_ card: ShopCard) {
        if isTakingFinalExam {
            showCurrentExamCard()
            return
        }
    ...
    private func handleRoundCompletion(for roundIndex: Int) {
        ...
        if nextRound < ShopCurriculum.rounds.count {
            progressStore.advanceToNextRoundIfNeeded()
            progressStore.unmarkTipSeen(forRound: nextRound)
            lastCardID = nil
            if ShopCurriculum.rounds[nextRound].isFinalExam {
    ...
    private func showTip(for roundIndex: Int) {
        phase = .showingTip(
            roundIndex: roundIndex,
            tips: ShopCurriculum.tips(forRound: roundIndex)
        )
    }

    private func selectNextCard(excluding excludedID: String? = nil) -> ShopCard? {
        ...
        if progressStore.currentSubRoundIndex == ShopGameConstants.mixedSubRoundIndex {
        ...
        var pool: [(card: ShopCard, weight: Int)] = []
        ...
        for card in unlearnedCurrent where card.id != avoidID {
            ...
            if progress.totalIncorrect > 0 && progress.consecutiveCorrect == 0 {
                weight = ShopGameConstants.strugglingCardWeight
            } else if progress.consecutiveCorrect > 0 {
                weight = ShopGameConstants.partialProgressWeight
            } else {
                weight = ShopGameConstants.currentRoundCardWeight
            }
            pool.append((card, weight))
        }

        if pool.isEmpty {
            pool = unlearnedCurrent.map { ($0, ShopGameConstants.currentRoundCardWeight) }
        }

        if progressStore.currentSubRoundIndex == ShopGameConstants.mixedSubRoundIndex {
            let reviewCards = ShopCurriculum.allCards.filter { reviewCard in
            ...
            for card in reviewCards {
                pool.append((card, ShopGameConstants.reviewCardWeight))
            }
        }
    ...
    private func isEarlierSubRound(_ card: ShopCard) -> Bool {
    ...
    private func weightedRandom(from pool: [(card: ShopCard, weight: Int)]) -> ShopCard? {
...
extension ShopGameViewModel {
    func multipleChoiceOptions(for card: ShopCard) -> [Int] {
        let answerLabel = card.answerLabel
        let pool = ShopConversion.distractorLabels(for: card).filter { $0 != answerLabel }
