//
//  HereToThereGameViewModel.swift
//  Metricize
//
...
enum HereToThereGamePhase: Equatable {
    case playing(HereToThereCard)
...
enum HereToThereAnswerFeedback: Equatable {
    case none
    case exact(answerLabel: String, card: HereToThereCard)
    case incorrect(correctLabel: String, card: HereToThereCard)
}

@Observable
@MainActor
final class HereToThereGameViewModel {
    let progressStore: HereToThereProgressStore
    let settings: AppSettingsStore

    private(set) var phase: HereToThereGamePhase = .playing(HereToThereCurriculum.rounds[0].cards[0])
    private(set) var feedback: HereToThereAnswerFeedback = .none
    private(set) var isSubmitting = false
    private(set) var activeChoiceLabels: [String]?
    private var activeCorrectChoiceIndex = 0
    private var lastCardID: String?

    init(progressStore: HereToThereProgressStore, settings: AppSettingsStore) {
    ...
    var currentRound: HereToThereRound {
        HereToThereCurriculum.rounds[progressStore.currentRoundIndex]
    }
    ...
    func submitMultipleChoice(guess: Int, for card: HereToThereCard) {
    ...
    private func resumeFinalExam() {
        if progressStore.shouldShowTipBeforeRound(HereToThereGameConstants.finalExamRoundIndex) {
            showTip(for: HereToThereGameConstants.finalExamRoundIndex)
            return
        }
    ...
    private func submitAnswer(guess: Int, for card: HereToThereCard) {
        ...
        let result = HereToThereConversion.evaluate(guess: guess, correctIndex: activeCorrectChoiceIndex)
    ...
    private func submitExamAnswer(guess: Int, for card: HereToThereCard) {
    ...
    private func presentFeedback(
        result: HereToThereConversion.AnswerResult,
        card: HereToThereCard,
    ...
    private func continueAfterLearningAnswer(for card: HereToThereCard) {
    ...
    private func presentFinalExamResult(from session: HereToThereFinalExamSession) {
        let passed = session.correctCount >= HereToThereGameConstants.examPassCorrectCount
        if passed {
    ...
    private func presentBatchReviewIfNeeded() -> Bool {
        ...
        phase = .showingBatchReview(
            roundIndex: roundIndex,
            subRoundIndex: subRoundIndex,
            items: HereToThereCurriculum.batchReviewItems(for: cards)
        )
        return true
    }

    private func transitionToCard(_ card: HereToThereCard) {
        if isTakingFinalExam {
            showCurrentExamCard()
            return
        }
    ...
    private func handleRoundCompletion(for roundIndex: Int) {
        ...
        if nextRound < HereToThereCurriculum.rounds.count {
            progressStore.advanceToNextRoundIfNeeded()
            progressStore.unmarkTipSeen(forRound: nextRound)
            lastCardID = nil
            if HereToThereCurriculum.rounds[nextRound].isFinalExam {
    ...
    private func showTip(for roundIndex: Int) {
        phase = .showingTip(
            roundIndex: roundIndex,
            tips: HereToThereCurriculum.tips(forRound: roundIndex)
        )
    }

    private func selectNextCard(excluding excludedID: String? = nil) -> HereToThereCard? {
        ...
        if progressStore.currentSubRoundIndex == HereToThereGameConstants.mixedSubRoundIndex {
        ...
        var pool: [(card: HereToThereCard, weight: Int)] = []
        ...
        for card in unlearnedCurrent where card.id != avoidID {
            ...
            if progress.totalIncorrect > 0 && progress.consecutiveCorrect == 0 {
                weight = HereToThereGameConstants.strugglingCardWeight
            } else if progress.consecutiveCorrect > 0 {
                weight = HereToThereGameConstants.partialProgressWeight
            } else {
                weight = HereToThereGameConstants.currentRoundCardWeight
            }
            pool.append((card, weight))
        }

        if pool.isEmpty {
            pool = unlearnedCurrent.map { ($0, HereToThereGameConstants.currentRoundCardWeight) }
        }

        if progressStore.currentSubRoundIndex == HereToThereGameConstants.mixedSubRoundIndex {
            let reviewCards = HereToThereCurriculum.allCards.filter { reviewCard in
            ...
            for card in reviewCards {
                pool.append((card, HereToThereGameConstants.reviewCardWeight))
            }
        }
    ...
    private func isEarlierSubRound(_ card: HereToThereCard) -> Bool {
    ...
    private func weightedRandom(from pool: [(card: HereToThereCard, weight: Int)]) -> HereToThereCard? {
...
extension HereToThereGameViewModel {
    func multipleChoiceOptions(for card: HereToThereCard) -> [Int] {
        let answerLabel = card.answerLabel
        let pool = HereToThereConversion.distractorLabels(for: card).filter { $0 != answerLabel }
