...
@Observable
final class GymProgressStore {
    private let storageKey = "metricize.gym.progress"
    ...
    private(set) var finalExamSession: GymFinalExamSession?
    ...
    func progress(for card: GymCard) -> CardProgress {
        progressByCardID[card.id] ?? CardProgress()
    }

    func recordAnswer(for card: GymCard, correct: Bool) {
        var progress = progress(for: card)
        let inMixedPractice = currentSubRoundIndex == GymGameConstants.mixedSubRoundIndex
    ...
    func startFinalExamSession(_ questions: [GymCard]) {
        finalExamSession = GymFinalExamSession(questions: questions)
        currentRoundIndex = GymGameConstants.finalExamRoundIndex
    ...
    func allCardsForSubRound(roundIndex: Int, subRoundIndex: Int) -> [GymCard] {
        GymCurriculum.cards(forRound: roundIndex, subRoundIndex: subRoundIndex)
    }

    func usesGradualRelease(subRoundIndex: Int) -> Bool {
        subRoundIndex != GymGameConstants.mixedSubRoundIndex
    }
    ...
    func ensureReleasedCountInitialized(roundIndex: Int, subRoundIndex: Int) {
        ...
        releasedCardCountByKey[key] = min(GymGameConstants.learningBatchSize, total)
        save()
    }
    ...
    func releasedCardCount(roundIndex: Int, subRoundIndex: Int) -> Int {
        ...
        let key = subRoundKey(roundIndex: roundIndex, subRoundIndex: subRoundIndex)
        let total = allCardsForSubRound(roundIndex: roundIndex, subRoundIndex: subRoundIndex).count
        return min(releasedCardCountByKey[key] ?? GymGameConstants.learningBatchSize, total)
    }

    func currentSubRoundCards() -> [GymCard] {
    ...
    func cardsForBatchReview(roundIndex: Int, subRoundIndex: Int) -> [GymCard] {
    ...
    func releaseNextBatchIfCurrentSetComplete(roundIndex: Int, subRoundIndex: Int) {
        ...
        let all = allCardsForSubRound(roundIndex: roundIndex, subRoundIndex: subRoundIndex)
        let released = releasedCardCount(roundIndex: roundIndex, subRoundIndex: subRoundIndex)
        releasedCardCountByKey[key] = min(released + GymGameConstants.learningBatchSize, all.count)
        save()
    }
    ...
    func currentReleasedCards(roundIndex: Int, subRoundIndex: Int) -> [GymCard] {
        let all = allCardsForSubRound(roundIndex: roundIndex, subRoundIndex: subRoundIndex)
        let released = releasedCardCount(roundIndex: roundIndex, subRoundIndex: subRoundIndex)
        return Array(all.prefix(released))
    }
    ...
    func isSubRoundComplete(_ roundIndex: Int, subRoundIndex: Int) -> Bool {
        ...
        if subRoundIndex == GymGameConstants.mixedSubRoundIndex {
    ...
    func isRoundComplete(_ roundIndex: Int) -> Bool {
        ...
        return (0..<GymGameConstants.subRoundsPerRound).allSatisfy {
            isSubRoundComplete(roundIndex, subRoundIndex: $0)
        }
    }

    func areLearningRoundsComplete() -> Bool {
        (0..<GymGameConstants.learningRoundCount).allSatisfy(isRoundComplete)
    }
    ...
    func learnedCountInCurrentSubRound() -> Int {
        ...
        if currentSubRoundIndex == GymGameConstants.mixedSubRoundIndex {
    ...
    func totalCountInCurrentSubRound() -> Int {
        if isFinalExamRound(currentRoundIndex) {
            return finalExamSession?.totalQuestions ?? GymGameConstants.examQuestionCount
        }
        if currentSubRoundIndex == GymGameConstants.mixedSubRoundIndex {
            return allCardsForSubRound(
                roundIndex: currentRoundIndex,
    ...
    func anchorCount(in roundIndex: Int) -> Int {
        GymCurriculum.rounds.first(where: { $0.index == roundIndex })?.anchorCount ?? 5
    }
    ...
    func advanceSubRoundIfNeeded() -> Bool {
        ...
        guard currentSubRoundIndex < GymGameConstants.subRoundsPerRound - 1 else { return false }
    ...
    func advanceToNextRoundIfNeeded() {
        ...
        if nextIndex < GymCurriculum.rounds.count {
    ...
    func normalizePosition() {
        ...
        while !isFinalExamRound(currentRoundIndex),
            ...
            if currentSubRoundIndex < GymGameConstants.subRoundsPerRound - 1 {
                currentSubRoundIndex += 1
            } else if currentRoundIndex < GymGameConstants.finalExamRoundIndex {
                currentRoundIndex += 1
                currentSubRoundIndex = 0
            } else {
                break
    ...
    private struct PersistedState: Codable {
        ...
        var finalExamSession: GymFinalExamSession?
