...
@Observable
final class KitchenProgressStore {
    private let storageKey = "metricize.kitchen.progress"
    ...
    private(set) var finalExamSession: KitchenFinalExamSession?
    ...
    func progress(for card: KitchenCard) -> CardProgress {
        progressByCardID[card.id] ?? CardProgress()
    }

    func recordAnswer(for card: KitchenCard, correct: Bool) {
        var progress = progress(for: card)
        let inMixedPractice = currentSubRoundIndex == KitchenGameConstants.mixedSubRoundIndex
    ...
    func startFinalExamSession(_ questions: [KitchenCard]) {
        finalExamSession = KitchenFinalExamSession(questions: questions)
        currentRoundIndex = KitchenGameConstants.finalExamRoundIndex
    ...
    func allCardsForSubRound(roundIndex: Int, subRoundIndex: Int) -> [KitchenCard] {
        KitchenCurriculum.cards(forRound: roundIndex, subRoundIndex: subRoundIndex)
    }

    func usesGradualRelease(subRoundIndex: Int) -> Bool {
        subRoundIndex != KitchenGameConstants.mixedSubRoundIndex
    }
    ...
    func ensureReleasedCountInitialized(roundIndex: Int, subRoundIndex: Int) {
        ...
        releasedCardCountByKey[key] = min(KitchenGameConstants.learningBatchSize, total)
        save()
    }
    ...
    func releasedCardCount(roundIndex: Int, subRoundIndex: Int) -> Int {
        ...
        let key = subRoundKey(roundIndex: roundIndex, subRoundIndex: subRoundIndex)
        let total = allCardsForSubRound(roundIndex: roundIndex, subRoundIndex: subRoundIndex).count
        return min(releasedCardCountByKey[key] ?? KitchenGameConstants.learningBatchSize, total)
    }

    func currentSubRoundCards() -> [KitchenCard] {
    ...
    func cardsForBatchReview(roundIndex: Int, subRoundIndex: Int) -> [KitchenCard] {
    ...
    func releaseNextBatchIfCurrentSetComplete(roundIndex: Int, subRoundIndex: Int) {
        ...
        let all = allCardsForSubRound(roundIndex: roundIndex, subRoundIndex: subRoundIndex)
        let released = releasedCardCount(roundIndex: roundIndex, subRoundIndex: subRoundIndex)
        releasedCardCountByKey[key] = min(released + KitchenGameConstants.learningBatchSize, all.count)
        save()
    }
    ...
    func currentReleasedCards(roundIndex: Int, subRoundIndex: Int) -> [KitchenCard] {
        let all = allCardsForSubRound(roundIndex: roundIndex, subRoundIndex: subRoundIndex)
        let released = releasedCardCount(roundIndex: roundIndex, subRoundIndex: subRoundIndex)
        return Array(all.prefix(released))
    }
    ...
    func currentExamCard() -> KitchenCard? {
    ...
    func isSubRoundComplete(_ roundIndex: Int, subRoundIndex: Int) -> Bool {
        ...
        if subRoundIndex == KitchenGameConstants.mixedSubRoundIndex {
    ...
    func isRoundComplete(_ roundIndex: Int) -> Bool {
        ...
        return (0..<KitchenGameConstants.subRoundsPerRound).allSatisfy {
            isSubRoundComplete(roundIndex, subRoundIndex: $0)
        }
    }

    func areLearningRoundsComplete() -> Bool {
        (0..<KitchenGameConstants.learningRoundCount).allSatisfy(isRoundComplete)
    }
    ...
    func learnedCountInCurrentSubRound() -> Int {
        ...
        if currentSubRoundIndex == KitchenGameConstants.mixedSubRoundIndex {
    ...
    func totalCountInCurrentSubRound() -> Int {
        if isFinalExamRound(currentRoundIndex) {
            return finalExamSession?.totalQuestions ?? KitchenGameConstants.examQuestionCount
        }
        if currentSubRoundIndex == KitchenGameConstants.mixedSubRoundIndex {
            return allCardsForSubRound(
                roundIndex: currentRoundIndex,
    ...
    func anchorCount(in roundIndex: Int) -> Int {
        KitchenCurriculum.rounds.first(where: { $0.index == roundIndex })?.anchorCount ?? 5
    }
    ...
    func advanceSubRoundIfNeeded() -> Bool {
        ...
        guard currentSubRoundIndex < KitchenGameConstants.subRoundsPerRound - 1 else { return false }
    ...
    func advanceToNextRoundIfNeeded() {
        ...
        if nextIndex < KitchenCurriculum.rounds.count {
    ...
    func normalizePosition() {
        ...
        while !isFinalExamRound(currentRoundIndex),
            ...
            if currentSubRoundIndex < KitchenGameConstants.subRoundsPerRound - 1 {
                currentSubRoundIndex += 1
            } else if currentRoundIndex < KitchenGameConstants.finalExamRoundIndex {
                currentRoundIndex += 1
                currentSubRoundIndex = 0
            } else {
                break
    ...
    private struct PersistedState: Codable {
        ...
        var finalExamSession: KitchenFinalExamSession?
